function clust  =  dbscan(points, epsilon, minPts)
	...
	%	Inputs:
	%
	%		points:	 A length N vector of points to cluster
	%
	...
	%	Outputs: 
	%		
	%		clust:  An N x 1 vector with cluster membership for each point. 
	...
	points = reshape(points, 1, []); 		% Ensure row vector
	...
	for i = 1:P
		...
		if ~isclustered(clust(pt))
			...
			seeds = regionQuery(distMat(:, pt), epsilon);
			if 	(numel(seeds) < minPoints)
				...
				clust = expandCluster(clust, seeds, clusterId, distMat, epsilon, minPts);
			end
		end
	end
end
...
function checkInputs(points, epsilon, minPts)
	...
	validateattributes(points, {'numeric'}, {'vector'}, mfilename, 'points', 1);
